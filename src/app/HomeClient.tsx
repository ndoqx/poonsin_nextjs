...
const BEST_SELLERS = [
  {
    ...
    span: "sm:col-span-2 md:col-span-2 md:row-span-2 h-[280px] sm:h-[380px] md:h-full",
...
const CarouselButton = ({
  ...
  <button
    onClick={onClick}
    aria-label={direction === 'next' ? 'ถัดไป' : 'ก่อนหน้า'}
...
export function HomeClient() {
  ...
  // Hero parallax — driven via refs/direct DOM writes (not React state) so scrolling
  // never triggers a re-render of the whole page tree.
  ...
  useEffect(() => {
    ...
    const applyParallax = () => {
      ...
      if (heroBgRef.current) {
        heroBgRef.current.style.transform = `scale(${1 + scrollY * 0.0002}) translateY(${scrollY * 0.08}px)`;
      }
      if (heroTextRef.current) {
        heroTextRef.current.style.opacity = `${Math.max(0, 1 - scrollY * 0.002)}`;
      }
      ticking = false;
    };
    const handleScroll = () => {
  ...
  return (
    <div className="bg-white text-gray-900 overflow-x-hidden selection:bg-amber-100 selection:text-gray-900">
      ...
      <section className="relative w-full h-[92vh] min-h-[560px] max-h-[900px] overflow-hidden">
        ...
        <div ref={heroTextRef} className="absolute bottom-0 left-0 z-10 p-8 md:p-14 lg:p-20 max-w-2xl">
      ...
      <section className="bg-[#1C1C1C] py-12 px-4 select-none">
        <div className="pb-8 text-center">
          <h2 className="text-[11px] font-extrabold tracking-normal uppercase text-[#B8882A]">
            สินค้ายอดฮิต
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 md:grid-rows-2 gap-4 max-w-7xl mx-auto h-auto md:h-[580px]">
          {BEST_SELLERS.map((product) => (
            <div
              ...
              className={`relative group overflow-hidden rounded-xl transition-all duration-300 shadow-md cursor-pointer bg-neutral-800 ${product.span}`}
            >
              <Image
                src={product.src}
                alt={product.title}
                ...
                className={`object-cover ${product.objectPosition} transition-transform duration-700 group-hover:scale-105`}
              />
              ...
              <div className="absolute bottom-5 left-5 right-5 text-white">
                <p className="text-base md:text-lg font-semibold">{product.title}</p>
                <p className="text-xs text-white/70 mt-1">{product.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeLightbox && (
        <div className="fixed inset-0 z-[999] flex flex-col items-center justify-center bg-black/90 p-4 md:p-6 backdrop-blur-md animate-in fade-in">
          ...
          <div className="w-full max-w-4xl flex flex-col items-center">
            {/* Intrinsic aspect ratio varies per photo — kept as a plain <img> to avoid distortion (see CarouselItem). */}
            <img
