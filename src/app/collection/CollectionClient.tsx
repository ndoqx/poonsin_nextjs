...
import { X, Phone, ChevronLeft, CheckCircle2, Star } from 'lucide-react';
import { AnimatePresence, motion } from 'framer-motion';
import { Reveal } from '@/components/ui/Reveal';
import { COLLECTION_PRODUCTS, COLLECTION_FILTERS, type CollectionItem } from '@/lib/collection-data';

function CollectionGrid() {
  const searchParams = useSearchParams();
  const initialCat = searchParams.get('category');
  const [filter, setFilter] = useState<string>(() =>
    initialCat && (COLLECTION_FILTERS as readonly string[]).includes(initialCat) ? initialCat : 'ทั้งหมด'
  );
  const [activeProduct, setActiveProduct] = useState<CollectionItem | null>(null);
  const [selectedImgIdx, setSelectedImgIdx] = useState(0);

  useEffect(() => {
    document.body.style.overflow = activeProduct ? 'hidden' : 'unset';
  ...
  const filteredItems = COLLECTION_PRODUCTS.filter((item) => filter === 'ทั้งหมด' || item.category === filter);
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <Reveal effect="fade-up" delay={200}>
            <div className="max-w-4xl mx-auto mt-12 md:mt-16 bg-[#FAF9F6] border border-gray-100/80 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-8 shadow-2xl shadow-gray-200/50 hover:shadow-gray-300/60 transition-all duration-500">
              <div className="relative w-full md:w-2/5 h-56 md:h-64 overflow-hidden rounded-2xl shadow-sm">
                <Image
                    ...
                    <div className="flex overflow-x-auto gap-2 pb-2 custom-scrollbar snap-x">
                      {activeProduct.images.map((img, idx) => (
                        <button
                          ...
                          className={`relative w-20 h-20 md:w-24 md:h-24 flex-shrink-0 snap-start rounded-lg md:rounded-xl overflow-hidden border-2 bg-white transition-all duration-200 ${idx === selectedImgIdx ? 'border-amber-500 opacity-100 shadow-md p-1' : 'border-transparent opacity-60 hover:opacity-100 p-1'}`}
                        >
                          <Image src={img} alt={`${activeProduct.name} ${idx + 1}`} fill sizes="96px" className="object-contain" />
                        </button>
                      ))}
                    </div>
                  )}
